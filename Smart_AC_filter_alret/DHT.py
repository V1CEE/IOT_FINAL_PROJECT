...
ClientName = "AC_Filter" + "#" + str(r)
DHT_topic = pub_topic
update_rate = manag_time * 1000  # in msec
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None):
        ...
        self.setWindowTitle('AC Filter')

        # Init QDockWidget objects
        self.connectionDock = ConnectionDock(self.mc)

        # Define the start time and the alert interval
        self.start_time = datetime.datetime.now()
        self.alert_interval = datetime.timedelta(seconds=5)  # Set this to the desired interval
    ...
    def update_data(self):
        ...
        # Check if the current time is past the start time plus the alert interval
        if datetime.datetime.now() >= self.start_time + self.alert_interval:
            # If it is, reset the start time to the current time
            self.start_time = datetime.datetime.now()

            # And publish an alert to the MQTT broker
            self.mc.publish_to(DHT_topic, 'Time to replace the AC filter')
